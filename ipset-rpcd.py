...
class Ipset_rpcd:
    ...
    def serve_forever(self):
        logging.info("Starting ipset JSON-RPC daemon at {bind}:{port}...".format(
            bind=self.args.bind,
            port=self.args.port,
        ))
        try:
    ...
    def _stop(user, mac, ip, role, timeout):
        logging.info((
    ...
    def _update_user(action, user, mac, ip, role, timeout):
        args = locals().copy()
    ...
    def _update_ipset(ipset, action, user, mac, ip, role, timeout):
